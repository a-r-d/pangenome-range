...
function statusDetail(): string {
  if (props.phase === "graph") {
    const counts = props.progress?.counts;
    const completedTiles = counts?.tiles ?? 0;
    const tileProgress =
      props.expectedTiles > 0
        ? `${completedTiles}/${props.expectedTiles} tiles`
        : `${completedTiles} tiles`;
    if (counts === undefined) {
      return `${formatBytes(props.bytes)} planned · preparing tile stream`;
    }
    return (
      `${formatBytes(props.bytes)} planned · ${tileProgress} · ` +
      `${counts.renderedNodes.toLocaleString()} nodes · ` +
      `${counts.renderedEdges.toLocaleString()} edges`
    );
  }
  return (
    `${formatBytes(props.bytes)}${props.queryTrace === undefined ? " planned" : " new"} · ` +
    `${props.tiles} tiles ${props.queryTrace === undefined ? "planned" : "ready"} · ` +
    formatMs(props.wallMs)
  );
}
</script>
...
<template>
  <section class="evidence" :class="{ open: props.open }" aria-label="Range and performance evidence">
    <button type="button" class="evidence-toggle" :data-loading="props.phase === 'graph'" :aria-expanded="props.open" @click="emit('toggle')"><span class="source-dot" :data-state="props.phase"></span><strong>{{ props.label }}</strong><span class="evidence-detail" aria-live="polite">{{ statusDetail() }}</span><svg viewBox="0 0 24 24" aria-hidden="true"><path :d="props.open ? 'm6 15 6-6 6 6' : 'm6 9 6 6 6-6'"></path></svg></button>
