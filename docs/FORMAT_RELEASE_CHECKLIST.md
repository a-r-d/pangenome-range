...
| Named identity preview | Final-layout rice/HPRC pilots pass below 1 GiB RSS and graph hashes/rounds match. Archive-wide storage overhead remains unmeasured; graph-only reads add 64 bytes and identity needs three serial extension rounds, so the mode remains experimental. | PASS WITH EXPERIMENTAL STATUS |
