...
| Timing integrity | Schema-12 encoder report plus retained results; source-cache, annotation, identity locate, output SHA-256, wall phases, and aggregate worker CPU are distinct. | PASS |
| Named identity preview | Final-layout rice/HPRC pilots pass below 1 GiB RSS and graph hashes/rounds match. The whole 30-assembly chicken archive adds archive-wide construction/size evidence and a VCF-backed two-tile UCD312 preset. Whole-HPRC named overhead remains unmeasured and identity remains experimental. | PASS WITH EXPERIMENTAL STATUS |
