# Rice named-membership production result
...
Named encoding added 540.90 ms (3.49%) and 1,884 KiB peak RSS over the anonymous control. Its fixed global catalog makes the one-tile archive 170.7% larger, but the same 222,267-byte increment is 0.0683% of the retained whole-chromosome archive. Graph-only semantics and dependency rounds are unchanged; registering the optional extension adds 64 fetched bytes. The identity query fetched 12,070 membership bytes and 10,586 catalog bytes.
