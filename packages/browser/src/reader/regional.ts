...
export function decodeRegionalPayload(
  ...
  if (version !== 4) {
    throw new UnsupportedRegionalPayloadVersionError(
      `regional payload v${version} is retained for Rust compatibility but is not supported by the TypeScript reader`,
    );
  }
  ...
  return {
    ...
    archiveOffset: options.archiveOffset ?? 0n,
    encodedLength,
    nodeIds: BigUint64Array.from(payload.nodeIds),
    nodeSequenceOffsets: sequences.offsets,
    nodeSequences: sequences.bytes,
    edges: BigUint64Array.from(payload.edges),
